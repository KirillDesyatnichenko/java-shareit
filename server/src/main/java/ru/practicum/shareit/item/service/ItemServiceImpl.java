...
public class ItemServiceImpl extends BaseService implements ItemService {
    ...
    public ItemDto createItem(Long userId, ItemInputDto inputDto) {
        ...
        if (inputDto.getRequestId() != null) {
            request = getOrThrow(requestRepository.findById(inputDto.getRequestId()),
                   "Запрос с id " + inputDto.getRequestId() + " не найден");
        }

        Item item = ItemMapper.toItem(inputDto, request);
        item.setOwner(owner);
